  ...
  actions: {
    async doLogin({ commit, dispatch }, { username, password }) {
      ...
      if (result === true) {
        ...
        // GAME 
        let QueUpdate = localStorage.getItem(server.QUEUPDATE);
        let StatusUpdate = localStorage.getItem(server.STATUSUPDATE);
        let ReviseMoldStatus = localStorage.getItem(server.REVISEMOLDSTATUS);
